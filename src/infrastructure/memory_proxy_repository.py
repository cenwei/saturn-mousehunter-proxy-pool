...
from domain import (
    IProxyRepository, IProxyFetcher, Proxy, ProxyPoolStats,
    ProxyStatus, MarketType, ProxyMode
)
from .proxy_fetchers import MockProxyFetcher
...
class MemoryProxyRepository(IProxyRepository):
    ...
    async def get_proxy_from_pool(self, proxy_type: str = "short") -> Optional[Proxy]:
        ...
        async with self._lock:
            ...
            # 活跃池为空，尝试备用池
            standby_proxies = [p for p in self.pools[self.standby_pool] if p.is_healthy()]
            if standby_proxies:
    ...
    async def get_stats(self) -> ProxyPoolStats:
        ...
        async with self._lock:
            ...
            return ProxyPoolStats(
                ...
                last_switch_time=datetime.fromtimestamp(self._last_rotate_ts).strftime("%Y-%m-%d %H:%M:%S"),
                ...
                success_rate=round(self._success_count / max(self._total_requests, 1) * 100, 2),
                last_fetch_time=(
                    datetime.fromtimestamp(self._last_fetch_time).strftime("%Y-%m-%d %H:%M:%S")
                    if self._last_fetch_time else "未记录"
                ),
        ...
        for batch_idx in range(self.batch_count):
            try:
                ...
                if batch:
                    ...
                    self.logger.debug(f"Fetched {len(batch)} proxies in batch {batch_idx + 1}")

            except Exception as e:
                self.logger.error(f"Failed to fetch proxies in batch {batch_idx + 1}: {e}")
