...
def get_cors_config() -> CORSConfig:
    """从环境变量获取CORS配置"""
    origins_str = os.getenv("PROXY_POOL_CORS_ORIGINS", "http://localhost:3000,http://localhost:8080")
    allow_origins = [origin.strip() for origin in origins_str.split(",") if origin.strip()]

    return CORSConfig(
        allow_origins=allow_origins,
        allow_credentials=os.getenv("PROXY_POOL_CORS_CREDENTIALS", "true").lower() == "true"
    )
...
def get_proxy_pool_config() -> ProxyPoolConfig:
    """从环境变量获取代理池配置"""
    # 默认海量代理URL
    default_hailiang_url = "http://api.hailiangip.com:8422/api/getIp?type=1&num=200&pid=-1&unbindTime=600&cid=-1&orderId=O25062920421786879509&time=1751266950&sign=d758b85241594a8b751147b511b836bf&noDuplicate=1&dataType=0&lineSeparator=0"
