    ...
    async def fetch_proxies(self, count: int = 20) -> List[str]:
        ...
        try:
            ...
            if isinstance(result, list):
                ...
                self.logger.warning("External fetch returned invalid data, using fallback")
                return await self._fallback.fetch_proxies(count)
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if "application/json" in content_type or response.text.strip().startswith("{"):
                        ...
                        if "未使用的IP太多" in response.text:
                            ...
                            return []
                        logger.error(f"海量代理API返回非JSON内容: {response.text[:200]}")
                        return []
                except Exception as e:
                    if "未使用的IP太多" in response.text:
                        logger.warning("海量代理API风控限流，主维护池需要休眠！")
                ...
                if data.get("code") == 0 and isinstance(data.get("data"), list):
                    ...
                    logger.warning(f"海量代理API返回异常内容: {data}")
            else:
                logger.warning(f"fetch_hailiang_proxy_ip 状态码异常: {response.status_code}")
